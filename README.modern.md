...
```ts
nz(["a", "b", "c"])
  .tap((x) => console.log(`consumed: ${x}`))
  .buffer(2)
  .tap((x) => console.log(`yielded: ${x}`)) // => consumed: a, consumed: b, yielded: a, consumed: c, yielded: b, yielded: c
```
...
nz(["Now I'm taking uuid-asdf-flkj and adding it to uuid-fslkj-alkjlsf."]).parse(/uuid-(?<id>\w+)-\w+/g, (m) => ({
  id: m.groups!.id,
}))
// yields: "Now I'm taking ", { id: "asdf" }, " and adding it to ", { id: "fslkj" }, "."

// Parse numbers from text
nz(["The answer is 42 and also 123"]).parse(/\d+/g, (m) => parseInt(m[0], 10))
...
```ts
function window<T, U, R = any>(
  source: Iterable<T, R>,
  fn: (ctx: { current: T; done: boolean; index: number; past: T[]; upcoming: T[] }) => { advance?: number; value: U },
